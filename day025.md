...
- Trường hợp **`O(n)`** của **add** xảy ra khi nó vượt quá kích thước ban đầu, lúc này nó sẽ tạo array mới và copy cái cũ sang
- Trường hợp chèn **`add(index, element)`** cũng mất **`O(n)`**

**LinkedList**

- Trường hợp chèn **`add(index, element)`** mất **`O(n/2)`**, chèn vào đầu mất **`O(1)`**, vào cuối mất **`O(n)`** do nó duyệt từ đầu tới cuối
